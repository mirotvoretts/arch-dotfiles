...
async function setupLocalEnvironment() {
	// 1. Установите локальный адрес контракта
	contractAddress = '0xCafac3dD18aC6c6e92c921884f9E4176737C052c' // Или ваш адрес

	// 2. Используйте локальный провайдер
	provider = new ethers.JsonRpcProvider('http://localhost:8545')

	// 3. Возьмите первый тестовый аккаунт
	await useLocalAccount(0)
}

// Получение данных проекта с сервера
...
function updateDonationSummary(amountETH) {
	...
	// Рассчитываем итоговую сумму (донат + комиссия сети)
...
document.getElementById('customAmount').addEventListener('input', e => {
	updateDonationSummary(e.target.value)
})

// При открытии модального окна
document.getElementById('sidebarDonateBtn').addEventListener('click', () => {
	// Получаем выбранную сумму (по умолчанию первая кнопка)
	const defaultAmount = document
		.querySelector('.donation-option')
		.getAttribute('data-amount')
	updateDonationSummary(defaultAmount)
})
