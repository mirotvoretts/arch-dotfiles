...
function setupAvatarUpload() {
	...
	// Сохраняем исходный аватар по умолчанию
	const defaultAvatar = avatarImage.src
	...
	avatarInput.addEventListener('change', e => {
		...
		if (file) {
			...
			reader.onload = function (event) {
				...
				// Показываем кнопку удаления после загрузки нового аватара
				if (avatarDelete) {
					avatarDelete.style.display = 'flex'
				}
			}
	...
	// Загрузка сохранённого аватара при инициализации
	const savedAvatar = localStorage.getItem('userAvatar')
	if (savedAvatar) {
		avatarImage.src = savedAvatar

		// Показываем кнопку удаления если аватар не дефолтный
		if (avatarDelete && avatarImage.src !== defaultAvatar) {
			avatarDelete.style.display = 'flex'
		}
	}
}
...
function setupSaveProfile() {
	...
	if (profileName)
		profileName.textContent = savedProfile.name || 'Имя пользователя'
	if (profileEmail)
		profileEmail.textContent = savedProfile.email || 'email@example.com'
	if (profilePhone)
		profilePhone.textContent = savedProfile.phone || 'Телефон не указан'
	...
	saveBtn.addEventListener('click', () => {
		...
		if (profilePhone)
			profilePhone.textContent = profileData.phone || 'Телефон не указан'
...
function setupProfileEdit() {
	...
	editBtn.addEventListener('click', () => {
		...
		document.getElementById('nameEdit').value =
			document.getElementById('nameView').textContent
		document.getElementById('emailEdit').value =
			document.getElementById('emailView').textContent
		document.getElementById('phoneEdit').value =
			document.getElementById('phoneView').textContent
	})
...
function loadProfileData() {
	...
	if (savedProfile) {
		...
		document.getElementById('emailView').textContent =
			savedProfile.email || 'Почта не указана'
		document.getElementById('phoneView').textContent =
			savedProfile.phone || 'Телефон не указан'
