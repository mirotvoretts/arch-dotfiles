...
contract DonationPool is Ownable {
    ...
    modifier onlyCampaignOwner(uint _campaignId) {
        require(campaigns[_campaignId].owner == msg.sender, NotACampaignOwner());
        _;
    }
    ...
    function withdrawFunds(uint _campaignId, address payable _recipient) external onlyCampaignOwner(_campaignId) {
        require(campaigns[_campaignId].currentAmount > 0, ZeroCampaignBalance());
