...
contract MyNFTCollection is ERC721, Ownable {
    ...
    function _validateMint(uint256 amount, address recipient) internal {
        if (amount == 0 || amount > Config.MAX_PER_TX) revert InvalidAmount();
        if (mintCount[recipient] + amount > Config.MAX_PER_WALLET)
            revert ExceedsWalletLimit();
        if (_nextTokenId + amount > MAX_SUPPLY) revert ExceedsMaxSupply();
        if (msg.value != amount * Config.MINT_PRICE) revert IncorrectETHValue();
    }
