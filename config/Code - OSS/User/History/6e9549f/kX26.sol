...
contract MyNFTCollectionTest is Test {
    ...
    function test_MintToDifferentRecipient() public {
        vm.deal(user1, TEST_FUNDS);
        vm.prank(user1);
        nft.mint{value: nft.MINT_PRICE() * 2}(2, user2);
        
        assertEq(nft.balanceOf(user2), 2, "Recipient should have NFTs");
    ...
    function test_BalanceOfAfterMultipleMints() public {
        vm.deal(user1, TEST_FUNDS);
        vm.prank(user1);
        nft.mint{value: nft.MINT_PRICE() * 2}(2, user1);

        vm.prank(user1);
        nft.mint{value: MINT_PRICE * 1}(1, user1);
        
        assertEq(nft.balanceOf(user1), 3, "Accumulated balance mismatch");
    }
