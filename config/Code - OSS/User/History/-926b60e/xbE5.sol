...
contract CharityCampaign is ReentrancyGuard {
    ...
    function executeRequest(
        ...
        uint256 requiredApprovals = (totalDonated * 30) / 100;
        if (request.approvalVotes <= requiredApprovals)
            revert InsufficientApprovals();

        uint256 available = totalDonated - totalSpent;
        ...
        delete spendingRequests[_requestId];

        (bool success, ) = recipient.call{value: amount}("");
        if (!success) revert TransferFailed();

        emit RequestExecuted(_requestId, amount);
    }
