...
contract CharityCampaign is ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => bool)) public requestVoters;
    ...
    function voteForRequest(uint256 _requestId) external onlyDonors {
        ...
        if (bytes(requestToVote.description).length == 0)
    ...
    function executeRequest(
        ...
        if (bytes(requestToExecute.description).length == 0)
            revert RequestDoesNotExist();
        ...
        if (requestToExecute.approvalVotes <= requiredApprovals) {
            revert InsufficientApprovals(
                requiredApprovals,
                requestToExecute.approvalVotes
            );
        }

        uint256 availableToSpend = totalDonated - totalSpent;
        if (requestToExecute.amount > availableToSpend)
            revert InsufficientFundsInContract();

        requestToExecute.executed = true;
        totalSpent += requestToExecute.amount;

        (bool success, ) = requestToExecute.recipient.call{
            value: requestToExecute.amount
        }("");
        if (!success) revert TransferFailed();
