...
contract CharityCampaign is ReentrancyGuard {
    ...
    event RequestCreated(uint256 indexed requestId, string description, uint96 amount, address indexed recipient);
    event RequestVoted(uint256 indexed requestId, address indexed voter, uint96 voteWeight);
    event RequestExecuted(uint256 indexed requestId, uint96 amount);
    ...
    constructor(address _owner, uint96 _goal, uint64 _durationInSeconds, address _platformRegistry) {
    ...
    function donate() public payable nonReentrant {
        ...
        uint96 feeAmount = (amount * feePercentage) / 100;
     ...
     */
    function createSpendingRequest(string calldata _description, uint96 _amount, address payable _recipient) external onlyCampaignOwner {
