...
contract PlatformRegistry is Ownable {
    ...
    uint96 public constant TOKEN_REWARD_RATE = 3000000000000000;
    ...
    constructor(address initialOwner, uint8 initialFeePercentage, address _tokenAddress) Ownable(initialOwner) {
        if (initialFeePercentage > MAX_FEE_PERCENTAGE) revert InvalidFeePercentage(); 
    ...
    function withdrawFees(address payable _to, uint96 _amount) external onlyOwner {
    ...
    function createCampaign(uint96 _goal, uint64 _durationInSeconds) external onlyVerified returns (address) {
        ...
        CharityCampaign campaign = new CharityCampaign(msg.sender, _goal, uint64(block.timestamp) + _durationInSeconds, address(this));
    ...
    function getCampaignInfo(uint64 _campaignId) external view returns (CampaignInfo memory) {
